...
async function onSubmit() {
  ...
  try {
    ...
    const res = await authApis.register(form.value);
    await auth.startNewSession(res.token);
      ...
      <n-card class="px-2 py-4 shadow-md">
        <n-form ref="formRef" :model="form" :rules="rules">
          <n-form-item
            label="Username"
            path="username"
            :validation-status="fieldErrors.username.validationStatus"
            :feedback="fieldErrors.username.feedback"
            class="transition-margin duration-150"
            :class="{ 'mb-4': fieldErrors.username.hasError }"
          >
          ...
          <n-form-item
            label="Firstname"
            path="name.first"
            :validation-status="fieldErrors.name.first.validationStatus"
            :feedback="fieldErrors.name.first.feedback"
            class="transition-margin duration-150"
            :class="{ 'mb-4': fieldErrors.name.first.hasError }"
          >
            <n-input v-model:value="form.name.first" placeholder="" />
          </n-form-item>
          <n-form-item
            label="Lastname"
            path="name.last"
            :validation-status="fieldErrors.name.last.validationStatus"
            :feedback="fieldErrors.name.last.feedback"
            class="transition-margin duration-150"
            :class="{ 'mb-4': fieldErrors.name.last.hasError }"
          >
            <n-input v-model:value="form.name.last" placeholder="" />
          </n-form-item>
          <n-form-item
            label="Email"
            path="email"
